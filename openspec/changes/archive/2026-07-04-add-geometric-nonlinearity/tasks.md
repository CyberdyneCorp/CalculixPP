...
- [x] 1.1 Declare `element_geometric_stiffness(ElementType, std::span<const Vec3> coords, const
      ...
      `element_stiffness`.
- [x] 1.2 Implement `element_geometric_stiffness` in `src/fem/element.cpp`: loop the SAME
...
- [x] 1.3 Unit-test on a single C3D8 under a prescribed uniaxial `σxx` that the block-diagonal
...
- [x] 2.1 Add `recover_gauss_stress(const Model&, const std::vector<Vec3>& u) →
...
- [x] 2.2 **[regression-lock]** Assert `recover_fields` output is byte-identical after the
...
- [x] 3.1 Declare `assemble_geometric_stiffness(const Model&, const
...
- [x] 3.2 Implement it in `src/fem/assembly.cpp` mirroring `assemble_linear_static` (`build_dof_map`,
      gather coords, `element_geometric_stiffness`, `scatter_tangent` through the transform,
      `flush_coo`); respect `element_active_mask`.
- [x] 3.3 Test that its COO free-DOF count and `dof_eq` match `assemble_linear_static` for the same
...
- [x] 4.1 **[upstream-gated for the SPARSE path]** File the SciPP target-selection ask (companion to
      SciPP#15): buckling pencil `(A = −K_geo, B = K)`, need smallest positive `λ = −1/θ`.
- [x] 4.2 Add a DEDICATED `extract_buckling_modes(const fem::LinearSystem& K, const fem::LinearSystem&
...
- [x] 4.3 Implement it in `src/numerics/eigensolution.cpp` via the DENSE generalized path first
...
- [x] 5.1 Add `struct BucklingReport { std::vector<Real> factors; … }` and
      ...
      3.x, 4.x.
- [x] 5.2 Assert the linear-static and Newton paths are byte-for-byte unchanged for a deck with no
      `*BUCKLE`/geometric step.

## 6. Parser + model (spec: input-deck-parsing [MODIFIED])

- [x] 6.1 Add `Procedure::Buckling` (after `Frequency`) and `int num_buckling_modes{0}` to
      `include/calculixpp/core/model.hpp`.
- [x] 6.2 In `src/io/inp_parser.cpp`: remove `*BUCKLE` from the deferred-card map; add
...
- [x] 6.3 Parser test: `beamb.inp` parses to `Procedure::Buckling` with `num_buckling_modes == 10`;
...
- [x] 7.1 Add a `Procedure::Buckling` dispatch branch in `apps/cli/main.cpp` (mirroring `Frequency`)
      calling `solve_buckling` and printing a `BUCKLING FACTOR OUTPUT` table (`MODE NO` /
      `BUCKLING FACTOR`).
- [x] 7.2 Add `buckling_result_dict` and a `Procedure::Buckling` case in `python/bindings.cpp`
...
- [x] 9.1 Run `beamb.inp` end-to-end; compare λ_1..λ_10 to `beamb.dat.ref`
...
- [x] 9.3 Python end-to-end: solve `beamb.inp` via the bindings and assert the returned factors match
      the CLI/`.dat` output.

## 10. OpenSpec + backlog hygiene

- [x] 10.1 On archive, mark the linear-`K_geo` slice of backlog row **2.2 `*BUCKLE`** done; leave

...
- [ ] 1.1 Add `GlobalSolution` (global mesh view + per-node displacement) to a new
  `core/submodel.hpp`: node ids/coords, element type + connectivity, `displacement` aligned
  with the global nodes.
- [ ] 1.2 Add `SubmodelSpec` (boundary node set name, global elset name, `TYPE=NODE`) and a
  `std::vector<SubmodelSpec> submodels` + a `driven` flag on `Spc` to the model.
- [ ] 1.3 Add `Procedure::Submodel`? — NOT needed; a submodel is an ordinary `*STATIC` step whose
...
- [ ] 2.1 `natural_coords(elem, coords, X)` — Newton inverse-isoparametric map: solve
  ...
  residual distance.
- [ ] 2.2 `find_host_element(global, X)` — search the global elements for the one containing X
  (ξ inside the reference domain within tolerance / smallest residual). Throw if none is found.
- [ ] 2.3 `interpolate_displacement(global, X)` — locate the host element, evaluate the shape
  functions at ξ, and return `Σ N_a(ξ) U_a` (the interpolated global displacement at X).
- [ ] 2.4 `solve_submodel(model, global)` — for every driven `*BOUNDARY, SUBMODEL` node, set its
  SPC value to the interpolated global displacement component, then run `solve_linear_static`.

## 3. Parser

- [ ] 3.1 `*SUBMODEL` card: `TYPE=NODE` (+ global elset) records a `SubmodelSpec`; register the
  boundary node set as driven. Remove `*SUBMODEL` from the deferred/rejected cards.
- [ ] 3.2 `*BOUNDARY, SUBMODEL`: mark the emitted SPCs as `driven` (value filled at solve time).
- [ ] 3.3 Reject `*SUBMODEL, TYPE=SURFACE` and `*CLOAD/*DSLOAD, SUBMODEL` with a clear
  "deferred" message (never silently ignore). [~]

## 4. Bindings + CLI

- [ ] 4.1 Python `solve_submodel(sub_text, global_ids, global_coords, global_conn, global_type,
...
- [ ] 4.2 CLI: `*SUBMODEL` decks need a global result file on disk (deferred), so the CLI reports a
  clear "provide the global solution via the Python API" message rather than mis-solving. [~]

## 5. Validation + regression

- [ ] 5.1 `tests/test_submodel.cpp`: build a coarse global beam deck + solution, cut a sub-region,
...
- [ ] 5.2 Wire `test_submodel` into `tests/CMakeLists.txt`; run `ctest` and the Python tests.

...
FullThermalSystem assemble_full_thermal(const Model& model, Real lambda) {
  return assemble_full_thermal(model, lambda,
                               std::vector<Real>(model.mesh.num_nodes(), 0.0));
}

FullThermalSystem assemble_full_thermal(const Model& model, Real lambda,
                                        const std::vector<Real>& node_temp) {
  const Mesh& mesh = model.mesh;
  const bool temp_dep = model.has_temp_dependent_thermal();
  const std::vector<Real> cond = temp_dep ? model.element_conductivity_at(node_temp)
                                          : model.element_conductivity();
  const std::vector<Real> rho_c = temp_dep ? model.element_heat_capacity_at(node_temp)
                                           : model.element_heat_capacity();
  const std::vector<bool> active = model.element_active_mask();
...
void recover_heat_flux(const Model& model, const std::vector<Real>& temperature,
  ...
  const Mesh& mesh = model.mesh;
  const std::size_t n_nodes = mesh.num_nodes();
  fields.heat_flux.assign(n_nodes, Vec3{0, 0, 0});
  fields.hfl_points.clear();
  ...
  for (std::size_t e = 0; e < mesh.num_elements(); ++e) {
    if (!active[e]) continue;  // *MODEL CHANGE, REMOVE: no heat flux
    const Element& elem = mesh.elements()[e];
    const int n = nodes_per_element(elem.type);
    gather_element(mesh, elem, n, nidx, coords);
    const Real k = cond[e];
    ...
    // q = -k grad(T) at each Gauss point: grad(T)[d] = sum_k g_k[d] * T_k.
    std::vector<Vec3> gp_flux(rule.size());
    for (std::size_t q = 0; q < rule.size(); ++q) {
      const Shape s = shape(elem.type, rule[q].xi, rule[q].et, rule[q].ze);
      physical_gradients(s, coords, g);
      Vec3 flux{0, 0, 0};
      for (int a = 0; a < n; ++a) {
        const Real Ta = temperature[static_cast<std::size_t>(nidx[static_cast<std::size_t>(a)])];
        for (int d = 0; d < 3; ++d)
          flux[static_cast<std::size_t>(d)] -=
              k * g[static_cast<std::size_t>(a)][static_cast<std::size_t>(d)] * Ta;
      }
      gp_flux[q] = flux;
      fields.hfl_points.push_back(
          HeatFluxPoint{elem.id, static_cast<int>(q) + 1, flux});
    }

    // Extrapolate GP flux to nodes (shared scheme) and average across elements.
    std::vector<std::array<Real, 3>> gp3(rule.size());
    for (std::size_t q = 0; q < rule.size(); ++q)
      gp3[q] = {gp_flux[q][0], gp_flux[q][1], gp_flux[q][2]};
    std::vector<std::array<Real, 3>> nodal(static_cast<std::size_t>(n));
    extrapolate_gp_to_nodes<3>(elem.type, n, gp3, nodal);
    for (int i = 0; i < n; ++i) {
      const std::size_t gi = static_cast<std::size_t>(nidx[static_cast<std::size_t>(i)]);
      for (int d = 0; d < 3; ++d)
        fields.heat_flux[gi][static_cast<std::size_t>(d)] +=
            nodal[static_cast<std::size_t>(i)][static_cast<std::size_t>(d)];
      ++count[gi];
    }
  }
  ...
  for (std::size_t i = 0; i < n_nodes; ++i)
    if (count[i] > 0)
      for (int d = 0; d < 3; ++d)
        fields.heat_flux[i][static_cast<std::size_t>(d)] /= count[i];
}
